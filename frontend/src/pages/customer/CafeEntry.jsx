...
export default function CafeEntry() {
  ...
  const [reservationHint, setReservationHint] = useState(null);
  const reservationCheckTimer = useRef(null);

  // When phone changes and order is dine-in, check for a reservation today
  const handlePhoneChange = (v) => {
    setForm((f) => ({ ...f, customer_phone: v }));
    clearTimeout(reservationCheckTimer.current);
    const digits = v.replace(/\D/g, '');
    if (digits.length >= 10 && form.order_type === 'dine-in') {
      reservationCheckTimer.current = setTimeout(async () => {
        try {
          const { data } = await checkReservationByPhone(slug, v.trim());
          if (data?.reservation) {
            setReservationHint(data.reservation);
            // Pre-fill name only if the field is still empty
            setForm((f) => ({
              ...f,
              customer_name: f.customer_name.trim() ? f.customer_name : data.reservation.customer_name,
            }));
          } else {
  ...
  const handleSubmit = (e) => {
    ...
    const session = {
      ...
      reservation_id: reservationHint?.id || null,
      cafe_name:      cafe?.name || '',
              ...
              {reservationHint && (
                <div className="mt-2 flex items-start gap-2 bg-green-50 border border-green-200 rounded-xl px-3 py-2.5">
                  ...
                  <div className="text-xs text-green-800 leading-relaxed">
                    <p className="font-semibold">Reservation found for today!</p>
                    <p>
                      {reservationHint.reserved_time?.slice(0, 5)} · Party of {reservationHint.party_size}
                      {reservationHint.table_label ? ` · Table ${reservationHint.table_label}` : ''}
                    </p>
                    <p className="text-green-600 mt-0.5">Your table is ready — go ahead and order.</p>
                  </div>
                </div>
              )}
            </div>
