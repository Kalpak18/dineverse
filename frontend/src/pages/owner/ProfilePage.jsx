      ...
      {activeTab === 'appearance' && (
        <div className="card space-y-6">
          ...
          <div className="border-t border-gray-100 pt-6 space-y-3">
            ...
            {/* Sound alert tune selector */}
            <div className="py-3 px-4 rounded-xl border bg-white border-gray-200 space-y-3">
              <div className="flex items-center gap-3">
                <span className="text-xl">🔊</span>
                <div>
                  <p className="text-sm font-medium text-gray-900">Order sound alert</p>
                  <p className="text-xs text-gray-400">Choose the tune that plays when a new order arrives</p>
                </div>
              </div>
              <div className="grid grid-cols-2 gap-2">
                {Object.entries(SOUND_TUNES).map(([id, tune]) => (
                  <button
                    key={id}
                    type="button"
                    onClick={() => {
                      setSelectedTune(id);
                      localStorage.setItem('dv_sound_tune', id);
                  ...
                  >
                    <span>{tune.emoji}</span>
                    <span className="truncate">{tune.label}</span>
                    {selectedTune === id && (
              ...
              <p className="text-xs text-gray-400">Tap any tune to preview it. Selection is saved on this device.</p>
            </div>
          </div>
        </div>
      )}
