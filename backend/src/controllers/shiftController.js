...
exports.closeShift = asyncHandler(async (req, res) => {
  ...
  const totals = await db.query(
    `SELECT
       ...
       COALESCE(SUM(final_amount - COALESCE(platform_fee,0) + COALESCE(platform_discount_amount,0)) FILTER (WHERE status='paid' AND payment_mode='cash'), 0) AS cash_sales,
       COALESCE(SUM(final_amount - COALESCE(platform_fee,0) + COALESCE(platform_discount_amount,0)) FILTER (WHERE status='paid' AND payment_mode='card'), 0) AS card_sales,
       COALESCE(SUM(final_amount - COALESCE(platform_fee,0) + COALESCE(platform_discount_amount,0)) FILTER (WHERE status='paid' AND payment_mode='upi'), 0)  AS upi_sales,
       COALESCE(SUM(final_amount - COALESCE(platform_fee,0) + COALESCE(platform_discount_amount,0)) FILTER (WHERE status='paid' AND payment_mode NOT IN ('cash','card','upi') AND payment_mode IS NOT NULL), 0) AS other_sales,
...
exports.getCurrentShift = asyncHandler(async (req, res) => {
  const { rows } = await db.query(
    `SELECT s.*,
            ...
            COALESCE(SUM(o.final_amount - COALESCE(o.platform_fee,0) + COALESCE(o.platform_discount_amount,0)) FILTER (WHERE o.status = 'paid'), 0)    AS live_revenue,
            COALESCE(SUM(o.final_amount - COALESCE(o.platform_fee,0) + COALESCE(o.platform_discount_amount,0)) FILTER (WHERE o.status='paid' AND o.payment_mode='cash'), 0) AS live_cash
     FROM shifts s
